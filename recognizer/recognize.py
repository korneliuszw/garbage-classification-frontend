...
def create_multipart_response(results, image_files, dir):
    ...
    def generate():
        ...
        yield json.dumps({"status": "success", "total_objects": len(results), "results": results}, indent=2)
        yield "\r\n"
    ...
    return Response(generate(), mimetype=f"multipart/form-data; boundary={boundary}", headers={"Content-Type": f"multipart/form-data; boundary={boundary}"})
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(delete=False) as temp_dir:
            ...
            if toggle_flag:
                ...
                results.append({"id": f"object_full", "detected_class": "none", "classified_as": predicted_class, "verdict": auto_map(predicted_class), "confidence": classification_confidence, "detection_confident": 0, "bbox": {}, "file_index": len(image_files)})
    ...
    return jsonify(
        {
            ...
            "endpoints": {"POST /recognize": "Upload image for recognition (returns multipart with files)", "POST /recognize-json": "Upload image for recognition (returns JSON only)", "GET /health": "Health check", "GET /": "This message"},
            "usage": {"multipart": 'Send POST to /recognize with form-data "file" field. Returns multipart response with metadata + image files', "json_only": "Send POST to /recognize-json for JSON-only response (no image files)"},
        }
    )
